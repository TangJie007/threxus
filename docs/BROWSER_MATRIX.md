...
4. Manual: open `/cube`, rotate, click, leave page (dispose), confirm no console errors after context restore simulation if available.

## Example coverage map

| Roadmap sample | Current coverage |
|----------------|------------------|
| 01-basic-scene | `examples/vue3` `/cube` |
...
| 06-pointer-interaction | `/cube` M8 click/hover |
| 07-on-demand-rendering | runtime unit tests (`renderMode`) |
| 08-postprocessing | `postprocessingFeature` unit tests |
| 09-context-restore | `webgl-context` unit tests |
