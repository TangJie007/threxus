...
/** 拉丝金属：水平细纹 + 轻微氧化斑 */
export function brushedMetal(size = 512, repeat = 1, tint = '#b9c2cc'): SurfaceMaps {
...
/** 网格线：地面基准网，替代 three 自带 GridHelper 的锯齿问题 */
export function gridLines(size = 512, repeat = 1): THREE.Texture {
...
/** 径向渐变光斑：用作自发光贴花、地面光圈 */
export function radialGlow(size = 256, inner = '#40e0ff', outer = 'rgba(64,224,255,0)'): THREE.Texture {
  return cached(`glow:${size}:${inner}`, () => {
    const { c, ctx } = makeCanvas(size)
    const g = ctx.createRadialGradient(size / 2, size / 2, 0, size / 2, size / 2, size / 2)
    g.addColorStop(0, inner)
    g.addColorStop(0.45, inner.replace(')', ',0.45)').replace('rgb', 'rgba'))
...
export function disposeProcedural(): void {
  cache.forEach((v) => {
    // 缓存里既有单张贴图，也有 { map/normalMap/roughnessMap } 组合
    const list = v && (v as THREE.Texture).isTexture ? [v as THREE.Texture] : Object.values(v as object)
    for (const t of list) if (t && (t as THREE.Texture).isTexture) (t as THREE.Texture).dispose()
  })
  cache.clear()
}
