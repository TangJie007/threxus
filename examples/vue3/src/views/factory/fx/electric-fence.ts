...
import type { ClipController, FenceController } from '../types'
 ...
 */
export function createElectricFence(opts: {
  width: number
  depth: number
  height?: number
  ...
  return {
    root,
    get alert() {
      return material.uniforms.uAlert.value > 0.5
    },
    set alert(v: boolean) {
      material.uniforms.uAlert.value = v ? 1 : 0
    },
    update(delta: number) {
...
export function createClipController(): ClipController {
  ...
  const apply = (): void => {
    const planes = enabled ? [plane] : null
