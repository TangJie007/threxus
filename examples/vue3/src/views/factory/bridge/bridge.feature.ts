/**
 * Factory 桥接：遥测 / 选中 / 标签 / 工具条 ↔ Vue 状态。
 */
...
import type { FactoryBridge, FactoryKpi, FactoryToggles } from '../types'
...
function computeKpi(devices: DeviceRecord[]): FactoryKpi {
  let run = 0
  let warn = 0
  let error = 0
  let loadSum = 0
...
export function createBridgeFeature(bridge: FactoryBridge) {
  return defineFeature({
    ...
    setup(context) {
      const scene = context.inject(FactorySceneService)
      const clip = context.inject(ClipService)
      ...
      bridge.devices = scene.devices
      bridge.toggles = { ...toggles }
      bridge.kpi = computeKpi(scene.devices)
      ...
      const focusDevice = (id: string): void => {
        ...
        scene.scanRing.focusAt(d.position.x, d.position.z)
        context.invalidate()
      }
      ...
      const setToggle = (key: keyof FactoryToggles, value: boolean): void => {
        toggles[key] = value
        bridge.toggles = { ...toggles }
        switch (key) {
          case 'outline':
          ...
          case 'ao':
            ...
            break
          case 'flow':
            scene.setFlowEnabled(value)
            break
          case 'fence':
            scene.setFenceAlert(value)
            break
          case 'clip':
      ...
      const hoverPreview = (id: string | null): void => {
        ...
        const d = id ? scene.findDevice(id) : undefined
      ...
      const telemetry = createMockTelemetry(scene.devices, 900)
      telemetry.onData((patch) => {
        for (const p of patch) {
          const d = scene.findDevice(p.id)
          if (!d) continue
          if (p.metrics) Object.assign(d.metrics, p.metrics)
          if (p.status && p.status !== d.status) {
            scene.applyStatus(d, p.status)
          }
        }
        bridge.devices = [...scene.devices]
        bridge.kpi = computeKpi(scene.devices)
